...
type Peer struct {
	ID      string
	kvStore map[Key]StampedValue
}

func (p *Peer) Read(key Key) StampedValue {
	return p.kvStore[key]
}

func (p *Peer) Write(key Key, value StampedValue) bool {
	if old, ok := p.kvStore[key]; ok && old.stamp.value < value.stamp.value {
		p.kvStore[key] = value
	} else {
		p.kvStore[key] = value
	}
